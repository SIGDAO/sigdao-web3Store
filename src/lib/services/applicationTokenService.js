...
export class ApplicationTokenService {
    ...
    async syncTokens() {
        ...
        try {
            ...
            this._dispatch(Events.Error, "Error by Tokens Sync")
        } finally {
    ...
    async _callContractMethod(token, passphraseOrWallet, methodHash, methodArgs) {
        try {
            ...
            //this._dispatch(Events.Error, e.message)
            this._dispatch(Events.Error, "Error by calling contract method")
            throw e
        } finally {
            finishLoading()
        }
    ...
    async registerToken(tokenData, passphraseOrWallet) {
        try {
            ...
            // this._dispatch(Events.Error, e.message)
            this._dispatch(Events.Error, "Error by token registration")
            // throw e
        } finally {
            finishLoading()
        }
