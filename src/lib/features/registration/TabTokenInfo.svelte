<script>
    ...
    import { MaxDescriptionLength, MaxNameLength, MaxTagCount, MaxTagLength, MaxUrlLength ,MaxDataLength} from './constants'
    ...
    $: isInvalidImage = !isValidImageFileUrl($registration$.img)
    ...
    $: projectImageLabel = `Project Image URL (${characterCount($registration$.img, MaxDataLength)}) approx. 720x400 px`
    ...
    async function handleFileUpload(e) {
        try {
            ...
            dispatchEvent(Events.Error, "Image File uploading failed ")
        } finally {
            isUploading = false
        }
    }
